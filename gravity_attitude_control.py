...
for i in tqdm(range(len(times))):
    ...
    # choose the body frame (z -> local down, x -> in plane containing local up and velocity vector
    bodyz = -localup / np.linalg.norm(localup)
    bodyy = -np.cross(localup, velt)
    bodyy *= 1.0 / np.linalg.norm(bodyy)
    bodyx = np.cross(bodyy, bodyz)
    ...
    # rotate around local up vector
    costheta, sintheta = np.cos(omega * times[i]), np.sin(omega * times[i])
    idx = i % 360
    bodyx, bodyy = costheta*bodyx + sintheta*bodyy, costheta*bodyy - sintheta*bodyx
    bodydcm = np.stack((bodyx, bodyy, bodyz))
...
ds_nadir.to_netcdf('simulations\\gravity_velocity.nc')
xarray_to_stk(ds_nadir, 'simulations\\gravity_velocity.a')

data_vars = {'quaternions': (['time', 'q'], q_rotate)}
ds_rotate = xr.Dataset(data_vars=data_vars, coords=coords, attrs=attrs)
ds_rotate.to_netcdf('simulations\\gravity_rotate.nc')
xarray_to_stk(ds_rotate, 'simulations\\gravity_rotate.a')
