...
class GeoMag:
    ...
    def GeoMag(self, dlat, dlon, h=0, time=datetime.date.today()): # latitude (decimal degrees), longitude (decimal degrees), altitude (feet), date
        #time = date('Y') + date('z')/365
        time = time.year+((time - datetime.date(time.year,1,1)).days/365.0)
        alt = h/3280.8399
...
def magnetic_field(date: datetime.date, r: np.ndarray):
    g = GeoMag()
